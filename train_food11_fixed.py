"""
使用Food-11数据集和EfficientNet-B4 + 注意力模块进行训练（修复版）
...
# 训练与验证函数
def train_model(model, train_loader, val_loader, device, epochs, optimizer, loss_fn, save_path, scheduler=None):
    """训练模型"""
    model.to(device)
    best_acc = 0
    train_losses = []
...
def main():
    ...
    # 创建数据加载器
    if torch.cuda.is_available():
        batch_size = 16  # GPU使用适中的batch size
        num_workers = 0  # 避免多进程问题
    else:
        batch_size = 8   # CPU使用较小的batch size
        num_workers = 0  # CPU避免多进程开销
    ...
    # 创建模型
    model = ResNet50_CBAM(num_classes=11)
    ...
    train_losses, train_accuracies, val_losses, val_accuracies = train_model(
        ...
        str(results_dir / "best_model.pth"), scheduler
    )
