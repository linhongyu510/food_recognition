...
def create_simulated_dataset():
    ...
    # 为每个类别创建一些示例图片
    for split in ['training/labeled', 'validation']:
        split_dir = data_dir / split
        split_dir.mkdir(parents=True, exist_ok=True)
        
        for i, class_name in enumerate(class_names):
            class_dir = split_dir / f"{i:02d}"
            class_dir.mkdir(exist_ok=True)
            
            # 创建一些示例文件（实际使用时需要真实图片）
            num_samples = 50 if split == 'training/labeled' else 20
            for j in range(num_samples):
                sample_file = class_dir / f"sample_{j:03d}.jpg"
                sample_file.touch()
    
    print(f"模拟数据集已创建: {data_dir}")
    return data_dir
...
def main():
    ...
    # 创建模拟数据集
    data_dir = create_simulated_dataset()
    
    # 创建数据集
    train_set = FoodDataset(str(data_dir / "training/labeled"), "train")
    val_set = FoodDataset(str(data_dir / "validation"), "val")
    ...
    train_losses, train_accuracies, val_losses, val_accuracies = train_model(
        model, train_loader, val_loader, device, 20, optimizer, loss_fn, 
