...
def train_val(model, train_loader, val_loader, device, epochs, optimizer, loss, thres, save_path):
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            optimizer.zero_grad()
            outputs = model(x).logits  # 获取模型输出
            train_bat_loss = loss(outputs, target)
            train_bat_loss.backward()
            optimizer.step()
            train_loss += train_bat_loss.item()
            train_acc += np.sum(np.argmax(outputs.detach().cpu().numpy(), axis=1) == target.cpu().numpy())
        ...
        with torch.no_grad():
            for batch_x, batch_y in val_loader:
                ...
                val_bat_loss = loss(outputs, target)
                val_loss += val_bat_loss.item()
                val_acc += np.sum(np.argmax(outputs.detach().cpu().numpy(), axis=1) == target.cpu().numpy())
...
model = ViTForImageClassification.from_pretrained("google/vit-base-patch16-224-in21k", num_labels=11)
...
train_val(model, train_loader, val_loader, no_label_loader, device, epochs=50, optimizer=optimizer, loss=loss,
          thres=0.6, save_path="model_best_vit.pth")
