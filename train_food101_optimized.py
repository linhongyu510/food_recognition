...
# 使用ResNet50 + CBAM（更稳定）
class ResNet50_CBAM_Optimized(nn.Module):
    def __init__(self, num_classes=101):
        super(ResNet50_CBAM_Optimized, self).__init__()
...
def main():
    """主函数"""
    print("=== 优化的Food-101数据集训练 - ResNet50 + CBAM ===")
    
    # 创建结果目录
    results_dir = Path("results_food101_optimized")
    ...
    # 创建模型
    model = ResNet50_CBAM_Optimized(num_classes=101)
    ...
    train_losses, train_accuracies, val_losses, val_accuracies = train_model_optimized(
        model, train_loader, val_loader, device, 100, optimizer, loss_fn, 
