...
def build_card(
    ...
    *,
    ...
    lines = [
        ...
        "tags:",
        tags,
        # model-index makes the numbers machine-readable and renders the Hub's
        # "Evaluation results" widget. Values come from metrics.json, like the
        # table below, so the widget cannot disagree with the prose.
        "model-index:",
        f"- name: {model_name}-{dataset.lower()}",
        "  results:",
        "  - task:",
        ...
        "    dataset:",
        f"      name: {dataset}",
        f"      type: {dataset.lower()}",
        "    metrics:",
        "    - type: accuracy",
        f"      value: {metrics['accuracy']}",
        "      name: Top-1 accuracy",
        "    - type: f1",
        f"      value: {metrics['macro_f1']}",
        "      name: Macro F1",
        "---",
        "",
